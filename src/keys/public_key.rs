...
impl<T: BigInt> PublicKey<T> {
    pub fn new(
        ...
        let hamming_f = hamming_weight as f64;
        let encryption_error = standard_deviation
            * (dimension_f * 8.0 * (2.0 as f64).sqrt()
                + 6.0 * dimension_f.sqrt()
                + 16.0 * ((hamming_weight as f64) * dimension_f).sqrt());
        let mut rng = rand::thread_rng();
        let gaussian = DiscreteGaussian::new(0.0, variance);
    ...
    pub fn encrypt_raw(&mut self, message: &CiphertextRing<T>) -> RawCiphertext<T> {
        let modulus = self.q.fast_exp(self.level_max) * self.q_0;
        let dimension = 2_usize.pow(self.dimension_exponent);

        let small_factor_coefficients = self.zod.sample_n(dimension);
        let small_factor = Polynomial::new(small_factor_coefficients)
            .modulo(modulus)
            .to_cyclotomic(self.dimension_exponent);
