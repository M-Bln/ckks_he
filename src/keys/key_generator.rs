...
//     let public_key_a = Polynomial::<T>::new(public_key_coefficients).modulo(modulus).to_cyclotomic(dimension_exponent);

//     let gaussian_sampler = DiscreteGaussian::new(0.0, variance);
//     let error_coefficients = gaussian_sampler.sample_n(dimension);
//     let error = Polynomial::<T>::new(error_coefficients).modulo(modulus).to_cyclotomic(dimension_exponent);
...
//     let eval_key_a =  Polynomial::<T>::new(eval_key_coefficients).modulo(modulus_eval).to_cyclotomic(dimension_exponent);
