...
impl<T: BigInt> EvaluationKey<T> {
	...
	let modulus = ct.raw.0.polynomial.ref_coefficients()[0].modulus;
    	...
    	ct.upper_bound_error = ct.upper_bound_error / factor.to_float() + self.noise.rescaling_noise;
    }
}
...
mod tests {
    ...
    fn test_encrypt_add_decrypt() {
        ...
        let (mut public_key, evaluation_key, secret_key) = generate_keys(dimension_exponent, q.clone(), level_max);
        ...
        let expected_message_coefficients = vec![I256::from(1 << 14); 2_usize.pow(dimension_exponent)];
	...
	println!("expected error: {:?}", added_ciphertext.upper_bound_error);
        ...
        for (expected, decrypted) in expected_message.polynomial.coefficients().iter().zip(decrypted_message.polynomial.coefficients().iter()) {
    ...
    fn test_encrypt_rescale_decrypt() {
        ...
        // Add the ciphertexts
        evaluation_key.rescale(&mut ciphertext, 1);

        // Decrypt the resulting ciphertext
        let decrypted_message = secret_key.decrypt(&ciphertext);

        // Create the expected message sum
        let expected_message_coefficients = vec![I256::from(1 << 13); 2_usize.pow(dimension_exponent)];
        let expected_message = Polynomial::new(expected_message_coefficients)
