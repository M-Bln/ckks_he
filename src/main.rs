...
fn main() {
    ...
    // Calculate error
    let expected_mul: Plaintext = plaintext1.iter().zip(plaintext2.iter()).map(|(c1, c2)| *c1 * *c2).collect();
    ...
    // Calculate error
    let expected_poly: Plaintext = plaintext1.iter().map(|c| complex_polynomial.eval(*c)).collect();
